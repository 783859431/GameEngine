...
void Pipeline::createPipelineLayout(VkDescriptorSetLayout* layouts, int layoutCount, VkPushConstantRange* ranges, int rangeCount)
{
    ...
    if (vkCreatePipelineLayout(Device::getInstance().device, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
...
{
    if(pipeline)
        vkDestroyPipeline(Device::getInstance().device, pipeline,nullptr);
    if(pipelineLayout)
        vkDestroyPipelineLayout(Device::getInstance().device, pipelineLayout, nullptr);
}
