...
layout(location = 2) in vec2 f_points[8];
layout(location = 10) flat in int f_count;
layout(location = 11) in float f_radius;
layout(location = 12) in float f_thickness;
...
void main()
{
    ...
    vec4 fillColor = 0.4f * borderColor;
    float dw = sdConvexPolygon(f_position, f_points, f_count);//点到线的距离有符号
