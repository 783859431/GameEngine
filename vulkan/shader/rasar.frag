...
{
    float r = length(uv - center);
    float R = 8.0+mod(87.0*pc.iTime, 80.0);
    return (0.5-0.5*cos(30.0*pc.iTime)) * SMOOTH(r,5.0)
...
void main() {
  ...
  vec2 iResolution = pc.iResolution;  
  vec2 fragCoord = gl_FragCoord.xy + vec2(-0.5*200, -0.5*600); //(inPos * 0.5 + 0.5) * pc.iResolution; 
