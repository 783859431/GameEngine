...
void SwapChain::createSwapChain()
{
    ...
    SwapChainSupportDetails swapChainSupport = querySwapChainSupport(Device::Instance().gpu, Device::Instance().surface);

    VkSurfaceFormatKHR surfaceFormat = chooseSwapSurfaceFormat(swapChainSupport.formats);
    VkPresentModeKHR presentMode = chooseSwapPresentMode(swapChainSupport.presentModes);
    VkExtent2D extent = chooseSwapExtent(swapChainSupport.capabilities, glWindow::Instance().getGLFWwindow());
    ...
    QueueFamilyIndices indices = Utils::findQueueFamilies(Device::Instance().gpu, Device::Instance().surface);
...
void SwapChain::recreateSwapchain()
{
    ...
    glm::vec2 wh = glWindow::Instance().GetFrameSize();
    while (wh.x == 0 || wh.y == 0) {
        wh = glWindow::Instance().GetFrameSize();
        glfwWaitEvents();
...
VkResult SwapChain::submitCmd(VkCommandBuffer* buffers, uint32_t* imageIndex)
{
    ...
    if (vkQueueSubmit(Device::Instance().graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS)
    {
    ...
    VkResult result = vkQueuePresentKHR(Device::Instance().presentQueue, &presentInfo); //GPU 将画面显示到平屏幕
